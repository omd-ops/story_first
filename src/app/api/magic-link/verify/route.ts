...
export async function GET(req: Request) {
  try {
    ...
    if (!token) {
      return NextResponse.redirect(new URL('/local?error=no-token', req.url));
    }

    const phone = verifyMagicToken(token, 3600); // 1 hour validity
    if (!phone) {
      return NextResponse.redirect(new URL('/local?error=invalid-token', req.url));
    }

    const safeRedirect =
      redirectParam && redirectParam.startsWith('/') ? redirectParam : getMagicLinkRedirectPath();
    const destination = new URL(safeRedirect, req.url);
    destination.searchParams.set('phone', phone);

    return NextResponse.redirect(destination);
  } catch (err) {
    return NextResponse.redirect(new URL('/local?error=server-error', req.url));
  }
}
